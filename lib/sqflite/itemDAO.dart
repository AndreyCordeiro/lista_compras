...
class ItemDAO {
  Future<bool> salvar(Item item) async {
    ...
    const sql = 'INSERT INTO item (nome, descricao, quantidade) VALUES (?,?,?)';
    var linhasAfetadas =
        await db.rawInsert(sql, [item.nome, item.descricao, item.quantidade]);
  ...
  Future<bool> alterar(Item item) async {
    ...
    var linhasAfetadas = await db
        .rawUpdate(sql, [item.nome, item.descricao, item.quantidade, item.id]);
    return linhasAfetadas > 0;
  }
